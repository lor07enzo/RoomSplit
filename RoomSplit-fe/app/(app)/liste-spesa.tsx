...
import { Home, Plus, ShoppingCart, Users } from 'lucide-react-native';
...
export default function ListeSpeseScreen() {
  ...
  const renderListaCard = (item: ListaSpesa, index: number) => (
    ...
    >
      <Text className="text-base font-semibold text-slate-800 dark:text-slate-200">{item.titolo}</Text>
  ...
  const renderSezioneGruppo = (sezione: { gruppo: Gruppo; items: ListaSpesa[] }, index: number) => (
    <View 
      ...
      className="mb-6 bg-white/40 dark:bg-slate-800/40 p-3 rounded-2xl border border-slate-200/30 dark:border-slate-700/50"
    >
      <View className="flex-row items-center gap-2 mb-3 px-1">
        ...
        <View className="flex-1">
          ...
          {sezione.gruppo?.codice_invito ? (
            <Text className="text-[10px] text-slate-400 dark:text-slate-500">Codice casa: {sezione.gruppo.codice_invito}</Text>
          ) : null}
        </View>
        <View className="bg-indigo-50 dark:bg-indigo-900/40 px-2 py-0.5 rounded-full">
          <Text className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400">{sezione.items.length}</Text>
        </View>
      </View>
